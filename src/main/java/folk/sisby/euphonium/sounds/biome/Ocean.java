...
public class Ocean implements ISoundType<BiomeSound> {
    ...
    public static final Predicate<RegistryEntry<Biome>> VALID_BIOME =
        holder -> holder.isIn(BiomeTags.IS_OCEAN) || holder.isIn(ConventionalBiomeTags.OCEAN);

    public Ocean() {
        DAY_SOUND = SoundHelper.sound(EuphoniumClient.id("biome.ocean.day"));
        NIGHT_SOUND = SoundHelper.sound(EuphoniumClient.id("biome.ocean.night"));
    }
